# Atomic Settlement Transaction - Deployment Guide

## Step 1: Apply Migration

Since Supabase CLI is not installed locally, you have two options:

### Option A: Use Supabase Studio (Recommended - Easiest)
...
### Option B: Install Supabase CLI (Alternative)
...
WHERE n.nspname = 'public' 
  AND p.proname = 'settle_entry';
```

You should see one row with the `settle_entry` function.

---

## Step 3: Test Function (Optional)

You can test the function works:

```sql
-- This should fail with "Entry not found" (expected)
SELECT settle_entry(
  '00000000-0000-0000-0000-000000000000'::uuid,  -- fake entry_id
  auth.uid(),  -- your user_id
  100,  -- amount
  CURRENT_DATE  -- date
);

-- Expected result: {"success": false, "error": "Entry not found or not accessible"}
```
...
Once you've applied the migration in Supabase Studio:
1. ✅ Run the SQL in Supabase Studio
2. ✅ Verify "Success. No rows returned"
3. ✅ Reply "Migration applied" and I'll proceed with swapping the actions files
